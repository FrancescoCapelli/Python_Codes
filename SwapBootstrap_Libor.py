...
for i in range(len(Coupon)-1):
    if Coupon.loc[i+1,'days'] < 1:
       Coupon.loc[i+1,'df'] = 1/(1+(Coupon.loc[i+1,'rate']/100)*Coupon.loc[i+1,'days'])*DFmm1
...
startingDates = data.loc[0:2,'date'].tolist()
date          =  data.loc[3,'date']
...
if os.path.exists(r"\\data01\primissu\ERQA\Analytical Tasks\Syndication\Pricing templates\PythonCodesandResults\DF_Libor.xls"):
  os.remove(r"\\data01\primissu\ERQA\Analytical Tasks\Syndication\Pricing templates\PythonCodesandResults\DF_Libor.xls")
else:
  print("The file has never been saved before")
  pass

writer = pd.ExcelWriter(r"\\data01\primissu\ERQA\Analytical Tasks\Syndication\Pricing templates\PythonCodesandResults\DF_Libor.xls")
...
#datelist2 = pd.date_range(start = final_zerocoupon.loc[0,'x'], end = data.loc[len(final_zerocoupon)-1,'x'] ).tolist()
#Interpolation=[]
...
#
#del final_zerocoupon['X']
#final_zerocoupon=final_zerocoupon.set_index('x')
#
