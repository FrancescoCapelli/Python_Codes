...
data_ois=pd.read_excel(r"\\data01\primissu\ERQA\Analytical Tasks\Syndication\0AIL2041 Syndication 09 July 2019\BootstrappingSwapCurve_francesco.xlsx",sheet_name='ois')
tableOfParameters=getTablesOfParameters(data_ois)

#########################################################################################################
#plot the spline
#########################################################################################################
datelist_ois = pd.date_range(start = data_ois.loc[0,'x'], end = data_ois.loc[len(data_ois)-1,'x'] ).tolist()
...
#
#
data=pd.read_excel(r"\\data01\primissu\ERQA\Analytical Tasks\Syndication\0AIL2041 Syndication 09 July 2019\BootstrappingSwapCurve_francesco.xlsx",sheet_name='ois')
data1=data.set_index('x')
G=pd.DataFrame(datelist_ois)
G['x']=A
G=G.set_index(0)

#ax = data1.plot(legend=False, figsize=(8, 5), x_compat=True,linestyle='',marker='o',color='y')
...
for i in range(len(Coupon)-1):
    if Coupon.loc[i+1,'yrs'] < 1:
       Coupon.loc[i+1,'df'] = 1/(1+(Coupon.loc[i+1,'y']/100)*Coupon.loc[i+1,'yrs'])*DFmm1
...
startingDates = data.loc[0:3,'x'].tolist()
date          =  data.loc[0,'x']
...
finalzerocoupon['y'] = Coupon['df']
tableOfParameters=getTablesOfParameters(finalzerocoupon)
datelist2_ois = pd.date_range(start = finalzerocoupon.loc[0,'x'], end = data.loc[len(finalzerocoupon)-1,'x'] ).tolist()
Interpolation=[]
